...
=======
            for session in u_sessions:
                if session["session_id"] == session_id:
                    return {"content_data": session["S_info"], "code": 200}
...
=======
        if user_data:
            user_info = user_data[0]
            try:
                clue, file_content = self.check_name(prompt_name, username, attachment_name)
            except Exception as e:
...
=======
                    history_data = session["S_info"]["content"]
                    try:
                        content = clue + content
                        content, response_ai = self.get_answer(file_content, content, dialogue_history, history_data)
                    except Exception as e:
                        return {"message": str(e), "status": 404}
                    history_data.append([content, response_ai])
                    session["S_info"] = {"content": history_data}
                    current_app.container.upsert_item(user_info)
                    session_info = {
                        "response_ai": response_ai,
                        "session_id": session_id
...
=======
        if input_file == "":
            ...
            input_data = f"{input_file[0] + '\n\n' if input_file[0] else ''}{input_data}"
            if input_file[1]:
                message = {"role": "user", "content": [{"type": "text", "text": input_data}]}
