  ...
  <div class="chat-area">
    ...
    <div class="chat-input-container">
      <div class="file-display-area">
        ...
        <div v-for="attachmentName in attachmentNames" :key="attachmentName" class="file-item">
      ...
      <div class="button-group">
        ...
        <label class="file-label" title="添付ファイル">
          ...
          <input type="file" id="fileInput" @change="uploadFile" style="display: none;" multiple>
        </label>
  ...
  methods: {
    ...
    async sendMessage() {
      ...
      try {
        ...
        if (this.attachmentNames && this.attachmentNames.length > 0) {
          requestBody.attachment_names = this.attachmentNames;
        }
    ...
    uploadFile(event) {
      ...
      Notification.error({
        title: '添付ファイル',
        message: '添付ファイルの合計サイズは5MBを超えません',
        duration: 3000,
        offset: 60
      });
      event.target.value = '';
      ...
      // 上传所有文件
      for (let i = 0; i < files.length; i++) {
      this.uploadFileToServer(files[i], 'attachment');
      }
      event.target.value = '';
    },
    saveMessage(event) {
      ...
      try {
        ...
        if (response.data.code === 200) {
          if (this.promptName === filename) {
            this.promptName = '';
          } else if (this.attachmentNames.includes(filename)) {
            this.attachmentNames = this.attachmentNames.filter(name => name !== filename);
          }
          Notification.success({
            title: 'ファイルの削除',
            message: 'ファイルを削除しました',
